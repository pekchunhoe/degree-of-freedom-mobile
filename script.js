...
function getMoleculePoints(type) {
  ...
  /* Non-linear / bent polyatomic model */
  return [
    {
      x: -38,
      y: 20,
...
function drawRotationIndicator(
   ...
   * X axis:
   * rotation plane = Y-Z
   * represented as vertical ellipse.
   */
  if (axis === "x") {
    ctx.beginPath();
    ctx.ellipse(
   ...
   * Y axis:
   * rotation plane = X-Z
   * represented as horizontal ellipse.
   */
  else if (axis === "y") {
    ctx.beginPath();
    ctx.ellipse(
...
function drawRotation() {
  ...
  /*
   * The selected X/Y/Z axis is the
   * actual rotation axis.
   *
   * X → rotation in Y-Z plane
   * Y → rotation in X-Z plane
   * Z → rotation in X-Y plane
   */
  drawAxisLine(
    ctx,
    width,
  ...
  /* Explicit rotational axis label */
  ctx.font =
    "700 14px sans-serif";
  ctx.fillText(
...
function updateRotationModes() {
  ...
  if (
    ...
    rotationModeList.innerHTML = `
      <div>Rₓ — rotation about X</div>
      <div>Rᵧ — rotation about Y</div>
      <div>2 perpendicular rotational modes</div>
    `;
  } else {
    rotationExplanation.textContent =
